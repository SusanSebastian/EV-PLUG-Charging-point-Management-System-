                ...
                if(mysqli_num_rows($getBookedPortQuery)>0){
                    ...
                    $portid = $getBookedPort['port_id'];
                        if($portid==$portidmain){
                            echo '<button style="background-color:red" id="btn'.$i.'" onclick="bookPort('.$i.', '.$portCount.'); return false" class="cardPort">
...
<?php
    ...
    $portname = $_SESSION['portData'];
    $fetchPortId = mysqli_query($db, "SELECT port_id from tbl_port where stationId='$sid' and portName='$portname'");
    $portIdResult = mysqli_fetch_assoc($fetchPortId);
    $portno = $portIdResult['port_id'];
    $_SESSION['portIdSelected'] = $portno;
    //echo "<script>alert('$text')</script>";
    if($portno != null){
        echo "<script>window.location.href='http://localhost/NEW EVPLUG/Customer/payment1.php'</script>";
    }
    else{
        echo '<script>swal("Select a port to continue");</script>';
    }
    }
?>
