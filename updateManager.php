...
$query = "SELECT * from tbl_station where status=1";
$result = mysqli_query($db,$query);
$mid=$_REQUEST['id'];
$query1 = "SELECT * from tbl_manager where managerId='$mid'";
...
<form action="" method="POST" name="addManager">
    ...
    <table>
        <tr>
          <td><input type="text" placeholder="Manager Name" placeholder="Name" value="<?php echo $result2['managerName']; ?>" id="mname" name="mname" onblur="return managernameValidate()"/>
          </td>
          <td><input type="text" placeholder="Contact" value="<?php echo $result2['managerContact']; ?>" maxlength="13" id="contact" name="contact" onblur="return cnoValidate()"/>
          </td>
          <tr>
          ...
          <?php 
          $sid = $result2['stationId'];
          $getStation = mysqli_query($db, "SELECT * from tbl_station where stationId='$sid'");
          $stationResult = mysqli_fetch_assoc($getStation);
          ?>
          <td><select name="station" id="station">
              <option selected hidden value="<?php echo $stationResult['stationId']; ?>"><?php echo $stationResult['stationName']; ?></option>
                <?php 
                while ($row = mysqli_fetch_array($result)) {
                ?>
                <option value="<?php echo $row['stationId'] ?>"><?php echo $row['stationName'];echo $row['place']; ?></option>
      ...
      <input type="submit" value="Update Manager" name="updateManager" id="updateManager"style="width: 12%; margin-left: 20%" onclick="return addmanagerValid()">
    </center>
  </form>
 </div>
...
//   // receive all input values from the form
  ...
  $stationId=$_POST['station'];
      ...
      $updatereg_query = "UPDATE tbl_manager SET managerName='$mname',managerContact='$contact',stationId=$stationId,status=1 where managerId=$mid";
      $reg_queryresult = mysqli_query($db,$updatereg_query);
      if($reg_queryresult){
        echo"<script Type='text/javascript'>alert('Manager Updated')</script>";
