    ...
    <div class="br-sideleft overflow-y-auto">
      ...
      <div class="br-sideleft-menu">
        ...
        <ul class="br-menu-sub nav flex-column">
          ...
          <li class="nav-item"><a href="#" class="nav-link">Add Service</a></li>
        </ul>
...
?>
<form method="POST" name="addWash">
    <center>
      <h1 style="margin-top: 10%; margin-left: 175px">New Service</h1>
    <table>
        <tr>
          <td><input type="text" placeholder="Service Type"id="serviceType" name="serviceType" required onblur="return WashType()"/>
          </td>
          <td><span id="WashType" style="color: red;"></span></td>
          <td><input type="text" placeholder="Price" id="price" name="price" required />
          </td>
        </tr>
      </table>
      <input type="submit" value="Add Service" name="addType" id="addType" style="width: 10%; margin-left: 20%">
    </center>
  </form>
 </div>
<!-- ==========Add Manager=============== -->
...
//   // receive all input values from the form
  ...
  $user_check_query = "SELECT * FROM tbl_service WHERE serviceType='$serviceType'";
  $checkqueryresult = mysqli_query($db, $user_check_query);
  $user = mysqli_fetch_assoc($checkqueryresult);
  if ($user){ // if user exists
    if ($user['serviceType'] == $washType) {
      array_push($errors, "Service Type already exists");
    }
    
  }
  if (count($errors) == 0) {
      $reg_query = "INSERT INTO tbl_service(serviceType,serviceCost,status)VALUES('$serviceType','$price',1)";
      $reg_queryresult = mysqli_query($db,$reg_query);
      if($reg_queryresult){
        echo"<script Type='text/javascript'>alert('Service Added')</script>";
        echo"<script>window.location.href='http://localhost/NEW EVPLUG/Admin/app/viewService.php';</script>";
      }
      else {
