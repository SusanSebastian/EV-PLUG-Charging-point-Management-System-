...
<form method="POST" name="addWash">
    ...
    <table>
        <tr>
          <td><input type="text" placeholder="Service Type"id="serviceType" name="serviceType" value="<?php echo $row['serviceType']; ?>" required onblur="return WashType()"/>
          </td>
          <td><span id="WashType" style="color: red;"></span></td>
          <td><input type="text" value="<?php echo $row['serviceCost']; ?>" placeholder="Price" id="price" name="price" required />
          </td>
        </tr>
      </table>
      <input type="submit" value="Update Service" name="addType" id="addType" style="width: 10%; margin-left: 20%">
    </center>
  </form>
 </div>
<!-- ==========Add Manager=============== -->
...
//   // receive all input values from the form
  ...
  if (count($errors) == 0) {
      $reg_query = "UPDATE tbl_service SET serviceType='$serviceType',serviceCost='$price', status=1 where serviceId='$id'";
      $reg_queryresult = mysqli_query($db,$reg_query);
      if($reg_queryresult){
        echo"<script Type='text/javascript'>alert('Service Updated')</script>";
